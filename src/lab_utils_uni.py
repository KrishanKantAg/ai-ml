...
# Go up one level to the project root and find the style file
style_path = os.path.join(os.path.dirname(current_dir), "deeplearning.mplstyle")
plt.style.use(style_path)

# This generates a color map(which binds number to colors)
n_bin = 5
...
def plt_intuition(x_train, y_train):
    ...
    def func(w=150):
        # since both w, and b are defined for this scope, we can have different values of fw_b/prediction(y)
        f_wb = np.dot(x_train, w) + tmp_b

        # this typically creates a new figure with two suplots
        # the constrained layout automatically adjust the spacing
        # the complete fig size is 8 x 4
        # 1 row and 2 columns, the constrained layout is used to automatically adjust the spacing so that the subplots text don't overlap
...
def plt_intuition_controlled(x_train, y_train):
    ...
    # Create slider with explicit continuous_update control
    w_slider = IntSlider(
        value=150,
        min=int(w_range[0]),
        max=int(w_range[1]),
        step=10,
        description="w:",
        continuous_update=False,  # This should work better
    )

    # Create interactive widget
    interactive_plot = interactive(update_plot, w=w_slider)
